...
def test_config_files():
    ...
    try:
        ...
        # 检查 sweeper 配置 (修复后的格式不再有 sweeper 包装器)
        if not sweeper_config.get("_target_"):
            print("✗ _target_ 未设置")
            return False

        if "storage_options" not in sweeper_config:
            print("✗ storage_options 未设置")
            return False

        print(f"✓ Sweeper 配置文件正确")
        print(f"  - _target_: {sweeper_config['_target_']}")
        print(f"  - storage_options: {sweeper_config['storage_options']}")
